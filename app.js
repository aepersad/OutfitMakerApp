...
/* ---------- Storage ---------- */
function loadItems() {
  const parsed = safeParse(localStorage.getItem(itemsKey(profile.id)), []);
...
function saveOutfitToDay(items, dateKey) {
  ...
  saveCalendarData();
  renderCalendar();
  renderFilteredOutfits(); // refresh plan-day button states
}
...
/* ── Generate all clothing combos (no shoes/outerwear) ── */
...
function generateOutfits(selected) {
  ...
  // Clothing items — existing pairing logic
  const shirts = ITEMS.filter(isShirt), layers = ITEMS.filter(isLayer),
        bottoms = ITEMS.filter(isBottom), dresses = ITEMS.filter(isDress);
  const out = [];
...
function renderOutfitCard(items, occasion) {
  ...
  if (completeItems.length) {
    ...
    cl.appendChild(cll);
    const clItems = document.createElement("div"); clItems.className = "complete-look-items";
    completeItems.forEach(ci => {
      ...
      thumb.appendChild(tImg); clItems.appendChild(thumb);
    });
    cl.appendChild(clItems); card.appendChild(cl);
  }
  ...
  getCurrentWeekDays().forEach(({ name, key: dayKey }) => {
    const btn = document.createElement("button");
    const isPlanned = CALENDAR_DATA[dayKey] && CALENDAR_DATA[dayKey].outfitKey === key;
    btn.className = "plan-day-btn" + (isPlanned ? " planned" : "");
...
surpriseMeBtn.addEventListener("click", () => {
  if (!ITEMS.length) { setOutfitMsg("Add items to your closet first."); return; }

  const seen = new Set(), all = [];
  ITEMS.forEach(item => {
    generateOutfits(item).forEach(combo => {
      const k = getOutfitKey(combo);
      if (!seen.has(k)) { seen.add(k); all.push(combo); }
    });
  });

  if (!all.length) { setOutfitMsg("Not enough items to match. Add more pieces!"); return; }
...
function renderCloset() {
  ...
  visible.forEach(item => {
    ...
    actions.appendChild(createBtn("Use", "primary", () => {
      const outfits = generateOutfits(item); renderOutfits(item, outfits);
    }));
